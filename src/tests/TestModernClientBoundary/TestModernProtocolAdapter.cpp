...
signals:
	...
	void deliveryBarrier();
	void deliveryBarrierReached();
};
...
void TestModernProtocolAdapter::preservesControlDeliveryOrderAndDiscardsCancelledAttempt() {
	...
	QSignalSpy cancellationSpy(&adapter, &adapters::ProtocolEventAdapter::attemptCancelled);
	...
	QObject::connect(&producer, &ProtocolEnvelopeProducer::deliveryBarrier, &adapter,
		[&producer] { emit producer.deliveryBarrierReached(); }, Qt::QueuedConnection);
	QSignalSpy barrierSpy(&producer, &ProtocolEnvelopeProducer::deliveryBarrierReached);
	QVERIFY(barrierSpy.isValid());
	...
	emit producer.deliveryBarrier();
	while (cancellationSpy.count() < static_cast< qsizetype >(1)) {
		QVERIFY(cancellationSpy.wait(1000));
	}
	while (barrierSpy.count() < static_cast< qsizetype >(1)) {
		QVERIFY(barrierSpy.wait(1000));
	}
	QCOMPARE(receiver.dispatchCount, 2);
}
